...
def check_port_allocation(num_ports: int = 100, start_port: int = 10000, stun_server_index: int = 0) -> None:
    ...
    logger.info("\n外部ポートの分布:")
    port_ranges = {}
    current_start = external_ports[0]
    current_end = external_ports[0]
    
    for i in range(1, len(external_ports)):
        if external_ports[i] == current_end + 1:
            current_end = external_ports[i]
        else:
            port_ranges[f"{current_start}-{current_end}"] = current_end - current_start + 1
            current_start = external_ports[i]
            current_end = external_ports[i]
    
    port_ranges[f"{current_start}-{current_end}"] = current_end - current_start + 1
    
    for port_range, count in port_ranges.items():
        logger.info(f"ポート範囲 {port_range}: {count}ポート")
